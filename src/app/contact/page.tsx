...
const contactFormSchema = z.object({
  ...
  email: z.string().email({ message: "Invalid email" }).refine((val) => {
  ...
  async function onSubmit(data: ContactFormValues) {
    ...
    try {
      const res = await fetch('/api/contact', {
        ...
        body: JSON.stringify({ ...data, preferredSlot: preferredSlot || undefined })
      })
        ...
        <div className="grid grid-cols-1 lg:grid-cols-12 gap-8 max-w-7xl mx-auto">
          ...
          <div className="lg:col-span-5">
            <div className="border-gradient p-[1px] rounded-3xl h-fit">
              <div className="bg-bg-secondary/60 backdrop-blur-xl rounded-[23px] p-8">

                {isSuccess ? (
                  <div className="text-center py-16 flex flex-col items-center gap-4">
                    <div className="w-16 h-16 rounded-full bg-gradient-to-br from-accent-primary/20 to-accent-violet/20 border border-accent-primary/30 flex items-center justify-center text-accent-success text-2xl mb-2">✓</div>
                    <h3 className="text-xl font-bold text-text-primary">Request Received</h3>
                    <p className="text-text-secondary text-sm leading-relaxed">
                      We'll confirm your demo slot and send a calendar invite to <span className="text-accent-primary">{form.getValues('email')}</span> shortly.
                    </p>
                    {preferredSlot && (
                  ...
                  <form onSubmit={form.handleSubmit(onSubmit)} className="flex flex-col gap-5">
                    ...
                    {/* Organisation */}
                    <div className="flex flex-col gap-1.5">
                      <label className="text-xs font-medium text-text-secondary uppercase tracking-wide">Health System / Organisation *</label>
                      <input {...form.register("company")} placeholder="Apollo Hospitals" className={INPUT_CLS} />
                    ...
                    <div className="flex flex-col gap-2">
                      ...
                      <div className="grid grid-cols-2 gap-2">
                        {['RCM Eligibility', 'Prior Auth', 'Scheduling', 'Billing & Collections', 'Care Gap Closure'].map((uc) => (
                          <label key={uc} className="flex items-center gap-2.5 p-3 rounded-lg border border-border-subtle bg-bg-primary/40 cursor-pointer hover:border-accent-primary/30 hover:bg-bg-primary/70 transition-all group">
                            <input
                              type="checkbox"
                              value={uc}
                            ...
                            />
                            <span className="text-xs text-text-secondary group-hover:text-text-primary transition-colors">{uc}</span>
                          </label>
                        ))}
                      </div>
                    ...
                    <div className="flex flex-col gap-1.5">
                      ...
                      <textarea {...form.register("message")} rows={3} placeholder="Any specific workflows, EHR systems, or payers you'd like us to demo against..." className={INPUT_CLS + " resize-none"} />
                    </div>

                    {/* Slot summary in form */}
                    {preferredSlot && (
          ...
          <div className="lg:col-span-7 flex flex-col gap-6">
            ...
            <DemoBookingWidget onSlotSelected={setPreferredSlot} />
